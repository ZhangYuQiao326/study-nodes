...
public:
    ...
    void qsort(vector<int>&nums, int l, int r){         // 1 传入引用
        ...
        // 分三块
        int i = l, left = l - 1, right = r+1;           // 3 快排主体
        while(i<right){
    ...
    int getKey(vector<int>&nums, int l, int r){
        // 通过随机数设置
        int ra = rand();
        return nums[ra % (r - l + 1) + l];  // 随机数 % nums长度 + 1
    }
};
...
<img src="C:\Users\zhang\AppData\Roaming\Typora\typora-user-images\image-20240317173504168.png" alt="image-20240317173504168" style="zoom: 33%;" />
