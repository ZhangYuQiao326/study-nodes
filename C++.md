...
​	**序列式容器**:强调值的排序，序列式容器中的每个元素均有固定的位置。（Sequence）
​	**关联式容器**:二叉树结构，各元素之间没有严格的物理上的顺序关系。（Associative）
...
void test01()
{
	...
	const char* str = "hello world"; // 隐式转换，带=的都是需要中间量接收，在赋值
...
* `int find(const string& str, int pos = 0) const;`              //查找子串， string
* `int find(const char* s, int pos = 0) const; `                     // c风格const char*
* `int find(const char* s, int pos, int n) const; `               //查找字符
* `int find(const char c, int pos = 0) const; `                       
* `int rfind(const string& str, int pos = npos) const;`      //从后往前
...
* find找到字符串后返回查找的第一个字符位置，找不到返回 `string::npos`
...
```cpp
string s('abc');
const char* ss = s.c_str(); // 获取首字符地址 

// string 和 const char* 类型区别
string： 打印所有字符  hello \0 world
const char*: 遇到 \0 就结束  hello
```

* char *

在C++的`std::string`类中，`c_str()`和`data()`都是用于获取字符串数据的成员函数，但它们在返回类型和一些行为上有一些细微的区别。

1. **返回类型:**
   - `c_str()`: 返回一个以null结尾的C风格字符串（const char*）的指针。
   - `data()`: 返回一个指向字符串数据的非常量指针（char*），但并不一定以null结尾。

2. **可修改性:**
   - `c_str()`: 返回的指针是指向常量字符的指针，意味着你不能通过这个指针修改字符串的内容。
   - `data()`: 返回的指针是指向非常量字符的指针，你可以通过这个指针修改字符串的内容。

3. **Null 终止符:**
   - `c_str()`: 确保返回的C风格字符串以null结尾。
   - `data()`: 不保证返回的字符串以null结尾，因此如果需要null终止，你应该自行添加。
...
int main() {
    ...
    // cStyleString 是以null结尾的常量字符数组

    char* nonConstData = &myString[0];
    // nonConstData 是指向字符串数据的非常量指针，但不一定以null结尾

    std::cout << "c_str(): " << cStyleString << std::endl;
    // 输出: Hello, World!
...
需要注意的是，使用`data()`函数时要确保你不会越界访问字符串的末尾，因为它不保证返回的字符串以null结尾。
...
```cpp
string s;
// cin >> s;  // hello world 报错：遇到空格、换行输入停止
getline(cin, s);  // hello world 成功，遇到换行停止
...
public:
    ...
    bool isPalindrome(string s) {
        ...
        while(left < right){
            while(left < right && !isNumOrAlp(s[left])) ++left;
            while(left < right && !isNumOrAlp(s[right])) --right;
...
// 嵌套容器，循环打印
for (map<int, map<string, string>>::iterator it = this->mOrderInfo.begin();it != this->mOrderInfo.end();it++) {
...
`std::pair` 是 C++ 标准库中的一个模板类，用于将两个值（元素）组合成一个对，即成对出现的数据。

我们可以通过创建 `std::pair` 对象来存储两个值，并使用 `first` 和 `second` 成员变量来访问这两个值。
...
```
`std::pair` 和 `std::map` 是 C++ 标准库中两个不同的概念和容器。

- `std::pair`：`std::pair` 是一个简单的模板类，用于将两个值（元素）组合成一个对。它通常用于表示一对相关的值，例如键值对、坐标等。`std::pair` 的对象只能存储两个值，并提供了 `first` 和 `second` 两个成员变量来访问这两个值。`std::pair` 主要用于临时存储和传递一对值，并没有提供像容器一样的插入、查找和删除等功能。

- `std::map`：`std::map` 是一个关联容器，用于存储键值对。它基于红黑树（一种自平衡二叉搜索树）实现，可以保持元素的有序性。`std::map` 中的每个元素都是一个键值对，其中的键是唯一的，用于进行快速查找。`std::map` 提供了丰富的成员函数和操作符，可以进行插入、查找、删除等操作。通过键值对的方式存储数据，`std::map` 适用于需要按照键进行查找和排序的场景。

主要区别：

- 存储方式：`std::pair` 只能存储两个值，用于临时存储一对相关的值；而 `std::map` 是一个关联容器，可以存储多个键值对，并提供丰富的操作方法。
- 功能和操作：`std::pair` 主要用于存储和传递一对值，没有提供像容器一样的插入、查找和删除等功能；`std::map` 提供了键值对的存储、查找和排序等功能，适用于按照键进行操作的场景。
- 底层实现：`std::pair` 是一个简单的模板类，底层实现并不复杂；`std::map` 基于红黑树实现，可以保持元素的有序性，并提供了高效的查找操作。

综上所述，`std::pair` 和 `std::map` 是两个不同的概念和容器，用途和功能也有所区别。选择使用哪个取决于具体的需求和场景。如果只需要临时存储一对相关的值，可以使用 `std::pair`；如果需要存储多个键值对并进行查找和排序，可以使用 `std::map`。
```
...
```cpp
int *ptr = new int(10);
std::unique_ptr<int> uptr (ptr); // 后面直接跟括号
...
public:
	...
	mystring(const char* str = "\0") : _str(new char[strlen(str) + 1]), _size(strlen(str)),_capacity(strlen(str))
	{
		// 拷贝字符串常量的值
		memcpy(_str, str, strlen(str)+1);
	...
	// 2 深拷贝构造(传统)
	/*mystring(const mystring& str) : _str(new char[str.size() + 1]), _size(str.size()), _capacity(str.capacity())
	{
		memcpy(_str, str.data(), str.size() + 1);
	}*/
	// （现代写法）
	// 封装允许类的成员函数访问同类的其他对象的私有成员
...
> 2. erase 删除it所指元素，会vs下中断报错，g++下不报错，但是忽略it，失效 。  解决：`it = vec.erase(it)` 返回删除元素后的下一个的迭代器
		...
		*/
		...
		void reserve(size_t n) {
			if (n > capacity()) {
				...
				if (_start) {
					for (size_t i = 0; i < num; ++i) {
						tmp[i] = _start[i];    // 本质调用string类的operator=深拷贝
					}
					delete[] _start;
				}
